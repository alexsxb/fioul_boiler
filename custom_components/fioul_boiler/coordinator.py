...
class FioulBoilerCoordinator(DataUpdateCoordinator[dict[str, Any]]):
    """
    Hauptlogik für:
    - Zustandsbestimmung (raw + debounced)
    - Brennerphasen
    - Verbrauchsberechnung nur am Ende einer Phase
    - PHC-Fehler
    - Abwesenheits-Fehler (>1h)
    - Anzeige von Durchfluss & thermischer Leistung
    """
    ...
    def __init__(self, hass: HomeAssistant, entry) -> None:
        ...
        # Last valid burn time (for >1h absence)
        self._burn_last_ok: Optional[datetime] = None

        # Real burn-phase tracking
    ...
    async def _async_update_data(self) -> dict[str, Any]:
        ...
        if self._last_state_filtered_change is None:
            ...
            # PHC pending setzen nach langem Pré-chauffage
        ...
        if self._phc_pending and self._phc_check_base_time:
            if now >= self._phc_check_base_time + timedelta(minutes=2):
                if state_filtered == STATE_BURN:
                    # überprüfen ob Burn 20s stabil
                    dur = (now - self._last_state_filtered_change).total_seconds()
                    if dur >= 20.0:
                        self._phc_error = False
        ...
        # -------------------------------------------------
        # ABSENCE LOGIK (>1h ohne gültigen Burn)
        # -------------------------------------------------
